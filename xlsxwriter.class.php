...
protected function tempFilename()
	...
	public function writeSheetHeader($sheet_name, array $header_types, $suppress_row = false, $col_options=null)
	{
		...
		if (!$suppress_row)
		{
			...
			foreach ($header_row as $v) {
                            ...
                            $cell_style_idx = empty($style) ? $sheet->columns[$c]['default_cell_style'] : $this->addCellStyle( $number_format , json_encode( isset($style[0]) ? (isset($style[$c]) ? $style[$c] : $style[0]) : $style) );
	...
	public function writeSheetRow($sheet_name, array $row, $row_options=null)
	{
		...
		foreach ($row as $v) {
			...
			$cell_style_idx = empty($style) ? $sheet->columns[$c]['default_cell_style'] : $this->addCellStyle( ( isset($style[$c]) ? (isset($style[$c][0]) ? $style[$c][0] : $number_format) : (isset($style[0][0]) ? $style[0][0] : $number_format)) , json_encode( isset($style[0]) ? ( isset($style[$c][1]) ? $style[$c][1] : (isset($style[$c]) ? $style[$c] : $style[0]) ) : $style) );
